...
syn keyword mysqlKeyword         call cascade change character check checksum column columns comment constraint create cross
...
syn keyword mysqlKeyword         default delayed delay_key_write delimiter delete desc describe distinct distinctrow drop duplicate
...
syn keyword mysqlKeyword         read references reload rename replace restrict return returns revoke row rows right
...
syn match mysqlComment           "#.*" display contains=mysqlTodo
syn match mysqlComment           "--\_s.*" display contains=mysqlTodo
syn keyword mysqlTodo            TODO FIXME XXX contained
